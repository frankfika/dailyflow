...
async function waitSettled(page, ms = 2500) {
  // Try to wait for any in-flight network, then a fixed settle window
...
async function main() {
  ...
  // Click on the first 2 task focus stars
  const stars = page.locator('button[aria-label*="star"], button[aria-label*="focus"], button[title*="star"], button[title*="focus"], svg.lucide-star').first();
  // Fallback: just try clicking on the focus text/icon
  for (let i = 0; i < 2; i++) {
    // Look for clickable focus indicator near each task
    const taskArea = page.locator('[data-testid^="task-"]').nth(i);
    if ((await taskArea.count()) > 0) {
      // Click the focus star inside
      const focus = taskArea.locator('svg.lucide-star, button').first();
      if ((await focus.count()) > 0) {
        await focus.click({ timeout: 1500 }).catch(() => {});
      }
    } else {
      // Try clicking directly on the star icon by index
