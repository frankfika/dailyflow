...
SOURCE_ID="$SOURCE_ID" python3 -c "import sys,json,os; d=json.load(sys.stdin); sid=os.environ['SOURCE_ID']; found=any(s['id']==sid for s in d['items']); assert found, 'captured source missing from inbox'; print(f'Total: {d[\"total\"]}, contains: {found}')" <<< "$INBOX"
...
echo "$PROC" | python3 -c "import sys,json; d=json.load(sys.stdin); assert d['fallback'] and d['empty']; print(f'fallback={d[\"fallback\"]} reason={d.get(\"fallbackReason\")} empty={d[\"empty\"]}')"
...
echo "$RELOAD" | python3 -c "import sys,json; c=json.load(sys.stdin)['commitment']; assert c['state']=='active'; print(f'title={c[\"title\"][:30]}... state={c[\"state\"]}')"
...
COM_ID="$COM_ID" python3 -c "import sys,json,os; d=json.load(sys.stdin); cid=os.environ['COM_ID']; items=d['plan']['items']; found=any(i['commitmentId']==cid for i in items); assert found, 'commitment missing from generated plan'; print(f'Plan items: {len(items)}, has commitment: {found}')" <<< "$PLAN"
...
echo "$REPLAN" | python3 -c "import sys,json; d=json.load(sys.stdin); assert d['plan']['availableMinutes']==120; print(f'Available: {d[\"plan\"][\"availableMinutes\"]}m')"
...
echo "$WAIT" | python3 -c "import sys,json; c=json.load(sys.stdin)['commitment']; assert c['state']=='waiting' and c.get('waitingOnText')=='Alex'; print(f'state={c[\"state\"]} waitingOnText={c.get(\"waitingOnText\")}')"
...
echo "$COMP" | python3 -c "import sys,json; d=json.load(sys.stdin); fup=d.get('followUpProposal'); assert d['commitment']['state']=='completed' and d['outcome']['id']; assert fup, 'follow-up proposal missing'; print(f'state={d[\"commitment\"][\"state\"]} outcomeId={d[\"outcome\"][\"id\"]} followUpProposal={fup[\"id\"] if fup else None} candidates={fup[\"candidateCount\"] if fup else 0}')"
...
if [ -n "$FUP_ID" ]; then
  APPLY=$(curl -s -X POST http://localhost:$PORT/api/v2/proposals/$FUP_ID/accept \
    -H "content-type: application/json" \
    -d '{"idempotencyKey":"e2e-follow-up-accept"}')
  echo "$APPLY" | python3 -c "import sys,json; d=json.load(sys.stdin); assert len(d.get('created', [])) > 0; print(f'created={len(d.get(\"created\", []))}')"
fi

echo "=== 10. Memory search ==="
SEARCH=$(curl -s "http://localhost:$PORT/api/v2/memory/search?q=Zhang")
echo "$SEARCH" | python3 -c "import sys,json; d=json.load(sys.stdin); assert d['hits'], 'memory search returned no hits'; print(f'Hits: {len(d[\"hits\"])}')"
...
[ -n "$EV_ID" ] || { echo "Evidence creation failed" >&2; exit 1; }
echo "Evidence: $EV_ID"
...
SEARCH_DEC=$(curl -s --get "http://localhost:$PORT/api/v2/memory/search" --data-urlencode "q=采用两档定价")
echo "$SEARCH_DEC" | python3 -c "import sys,json; d=json.load(sys.stdin); types=[h['type'] for h in d['hits']]; assert 'decision' in types, 'decision missing from memory search'; print(f'Hits: {len(d[\"hits\"])}, types={set(types)}')"
...
echo "$CTX" | python3 -c "import sys,json; d=json.load(sys.stdin); r=d['related']; assert r['evidence'] and r['sourceItems']; print(f'decisions={len(r[\"decisions\"])} evidence={len(r[\"evidence\"])} sources={len(r[\"sourceItems\"])}')"

echo "=== 11. Connectors blocked ==="
CONN=$(curl -s http://localhost:$PORT/api/v2/connectors)
echo "$CONN" | python3 -c "import sys,json; d=json.load(sys.stdin); assert d.get('error', {}).get('code') == 'not_found', 'disabled connector API was accessible'; print('Connector API correctly blocked')"

echo "=== 12. Index rebuild ==="
STATUS=$(curl -s http://localhost:$PORT/api/v2/status)
echo "$STATUS" | python3 -c "import sys,json; d=json.load(sys.stdin); assert d['index']['scanned'] > 0 and d['index']['entities'] > 0; print(f'Index: scanned={d[\"index\"][\"scanned\"]} entities={d[\"index\"][\"entities\"]}')"
...
echo "$COMP2" | python3 -c "import sys,json; d=json.load(sys.stdin); assert d['commitment']['state']=='completed'; print(f'After complete: state={d[\"commitment\"][\"state\"]}')"
RESUME=$(curl -s -X POST http://localhost:$PORT/api/v2/commitments/$NEW_ID/resume -H "content-type: application/json" -d '{}')
echo "$RESUME" | python3 -c "import sys,json; d=json.load(sys.stdin); assert d.get('error', {}).get('code')=='invalid_transition'; print(f'Resume attempt: {d}')"
