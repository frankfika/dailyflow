      ...
      <div className="w-full max-w-lg rounded-2xl border border-border bg-surface-elevated shadow-2xl" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="space-y-4 px-5 py-4">
          ...
          <div className="rounded-xl border border-border bg-background/60 p-3">
            ...
            <div className="mt-2 flex items-start gap-2 text-[12px] leading-5 text-text-muted"><LockKeyhole className="mt-0.5 h-3.5 w-3.5 shrink-0" />{zh ? '只发送下列已选类别；API Key、完整工作区和隐藏思维链不会进入上下文。' : 'Only selected categories are sent. API keys, the full workspace, and hidden reasoning are excluded.'}</div>
          </div>
...
function ContextStat({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border border-border bg-background/60 p-3"><div className="text-lg font-semibold tabular-nums text-text-heading">{value}</div><div className="mt-0.5 text-[11px] text-text-muted">{label}</div></div>;
}
