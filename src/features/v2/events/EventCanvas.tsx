...
const COPY: Record<'en' | 'zh', Copy> = {
  en: { child: 'Child', today: 'Today', date: 'Date', more: 'More', addStep: 'Add step', cancel: 'Cancel', removeDay: 'Remove from day', deleteNode: 'Delete node', empty: 'Start by adding the first step.', },
  zh: { child: '子节点', today: '今天', date: '日期', more: '更多', addStep: '添加步骤', cancel: '取消', removeDay: '移出日程', deleteNode: '删除节点', empty: '从添加第一个步骤开始。', },
};
...
interface EventCanvasProps {
  ...
  onAddChild: (parentId: string, text: string) => Promise<void>;
...
export function EventCanvas({
  ...
  useEffect(() => {
    if (focusedNodeId && event.nodes.some((node) => node.id === focusedNodeId)) setSelectedId(focusedNodeId);
  }, [event.nodes, focusedNodeId]);

  useEffect(() => {
    if (selectedId && !event.nodes.some((node) => node.id === selectedId)) setSelectedId(event.rootNodeId);
  ...
  async function submitChild() {
    ...
    await run(() => onAddChild(selected.id, childText.trim()));
    setChildText('');
    setAddingChild(false);
  }

  async function submitRename(node: EventNode) {
    if (!editText.trim() || editText.trim() === node.text) {
      setEditingId(null);
      return;
    }
    await run(() => onRename(node.id, editText.trim()));
    setEditingId(null);
  }
  ...
  return (
    <div className="relative h-full min-h-0 overflow-auto bg-[#f8faf9] outline-none dark:bg-[#111716]" data-testid="event-canvas" tabIndex={0} onKeyDown={handleCanvasKeyDown} onClick={() => { setMoreOpen(false); setDateOpen(false); }}>
          ...
          return (
            <div key={node.id} className="absolute" style={{ left: node.canvasX, top: node.canvasY, width: NODE_W }} data-testid={`event-node-${node.id}`}>
              <button
                ...
                className={`flex min-h-[58px] w-full items-center gap-2 rounded-xl border px-3 py-2 text-left transition ${isSelected ? 'border-[#23877B] bg-white ring-2 ring-[#23877B]/15 dark:bg-gray-900' : 'border-gray-200 bg-white hover:border-gray-300 dark:border-gray-700 dark:bg-gray-900'} ${node.id === event.rootNodeId ? 'font-semibold' : ''}`}
              ...
              >
                ...
                {editingId === node.id ? (
                  <input
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter') void submitRename(node); if (e.key === 'Escape') setEditingId(null); }}
      ...
      {selected && (
        <div className="sticky bottom-5 z-10 mx-auto flex w-fit max-w-[calc(100%-2rem)] items-center gap-1 rounded-xl border border-gray-200 bg-white p-1.5 shadow-lg dark:border-gray-700 dark:bg-gray-900" data-testid="event-node-toolbar" onClick={(e) => e.stopPropagation()}>
          {addingChild ? (
            ...
            <>
              <ToolbarButton icon={<Plus className="h-4 w-4" />} label={copy.child} onClick={() => setAddingChild(true)} />
      ...
      {!event.nodes.length && <div className="absolute inset-0 grid place-items-center text-sm text-gray-400">{copy.empty}</div>}
    </div>
  );
}
