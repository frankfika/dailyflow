          ...
          >
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto mb-5">
              {preview.tasksToMigrate.map((t, i) => (
                <div key={i} className="flex items-center gap-2.5 text-sm text-text-main py-1">
                  <span className="w-1.5 h-1.5 rounded bg-accent shrink-0" />
                  <span className="flex-1 truncate">{t.title}</span>
                  {t.source_date && <span className="text-xs text-text-muted shrink-0">{t.source_date}</span>}
                </div>
              ))}
            </div>
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-2 rounded-md bg-accent text-white text-sm font-medium hover:bg-accent/90 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
