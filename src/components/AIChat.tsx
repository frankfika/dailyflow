...
export function AIChat({ language, tasks, notes, filesMap, showToast, initialDraft, onDraftConsumed }: AIChatProps) {
  ...
  return (
    <div className="h-full flex bg-background">
      {/* —— Left: sessions —— */}
      <aside className="w-[260px] flex flex-col border-r border-border bg-surface">
        ...
        {activeSession && activeSession.contextItems.length > 0 && (
          <div className="px-8 py-2 border-b border-border bg-background flex items-center gap-1.5 flex-wrap">
            ...
            {activeSession.contextItems.map(item => (
              <span
                key={item.id}
                className="inline-flex items-center gap-1 px-2 py-0.5 text-[11px] bg-surface border border-border rounded-full"
              >
                        ...
                        <>
                          {skills.map(skill => (
                            ...
                            >
                              ...
                              <div className="text-[10px] text-text-muted truncate">{skill.prompt.slice(0, 60)}…</div>
                            </button>
                          ))}
                          <button
                            onClick={() => { setShowSkillMenu(false); setShowSettings(true); }}
