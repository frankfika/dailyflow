 ...
 * outbound traffic and which toggle controls it. The data is
...
const ENTRIES: PrivacyEntry[] = [
  ...
  },
  {
    ...
    description: '音频上传到 OpenAI Whisper（默认） / 本地 whisper.cpp（推荐）',
    descriptionEn: 'Audio uploaded to OpenAI Whisper (default) / local whisper.cpp (recommended)',
    togglesHere: true,
    destinations: ['OpenAI Whisper', 'whisper.cpp (本地)'],
    icon: 'mic',
    alwaysOn: false,
  },
  {
      ...
      <div className="flex items-start gap-3 rounded-lg border border-[var(--color-accent)]/20 bg-[var(--color-accent-light)] p-4">
        ...
        <div>
          ...
          <p className="mt-1 text-[12px] leading-relaxed text-text-main">
            {language === 'zh'
              ? 'DailyFlow 框架本身从不主动上传你的文件。下方每一类外发请求都有显式开关。AI 调用必须由你选择模型 + 发起对话才会发生。'
              : 'The DailyFlow framework never proactively uploads your files. Each outbound category below has an explicit toggle. AI calls only happen when you choose a model and start a conversation.'}
          </p>
        </div>
      </div>
